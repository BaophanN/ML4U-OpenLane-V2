...
class M2BevNeck(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.with_cp and x.requires_grad:
            ...
            x = _inner_forward(x)
        # print('3.m2bev neck after inner forward',x.shape)
        if self.is_transpose:
            # Anchor3DHead axis order is (y, x).
            # print ('3.m2bevneck transpose final',x.transpose(-1,-2).shape)
            return [x.transpose(-1, -2)]

        else:
            # print('4.m2bevneck not transposed final',x.shape)
            return [x]
