...
class OpenLaneV2_subset_A_LaneSegNet_Dataset(Custom3DDataset):
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            ...
            for cam_name, cam_info in info['pose'].items(): 
                #### Compute ego2sensor, sensor2ego 
                # sweep ego to global

                print('pose: ',cam_info)
                # 3x3 matrix in openlane v2 
                w, x, y = cam_info['rotation']
                ego2global_rot = torch.Tensor(
                    Quaternion(w, x, y).rotation_matrix)
                ego2global_tran = torch.Tensor(
                    cam_info['translation'])
                ego2global = ego2global_rot.new_zeros((4, 4))
                ego2global[3, 3] = 1
                ego2global[:3, :3] = ego2global_rot
                ego2global[:3, -1] = ego2global_tran # Final is the 3x4 matrix 
                ego2globals.append(ego2global)
            for cam_name, cam_info in info['sensor'].items():
                print('sensor: ', cam_info)
                w, x, y = cam_info['extrinsic']['rotation'] # sensor2ego rotation 
                ...
                img_augs = self.sample_augmentation(
                    H=img.height, W=img.width, flip=flip, scale=None)
        ...
        can_bus[:3] = info['pose']['translation'] # translation 
        can_bus[3:7] = rotation                   # rotation 
